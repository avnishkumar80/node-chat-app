...
const {generateMessages,getLocation} = require('./utils/messages')
...
io.on('connection',(socket)=>{
    ...
    socket.on('sendLocation',(coords,callback)=>{
        const user = getUser(socket.id)
        io.to(user.room).emit('location',getLocation(user.username,`https://google.com/maps?q=${coords.latitude},${coords.longitude}`))
        callback()
    })
